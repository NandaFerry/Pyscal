...
class Parser():
    ...
    def sinalizaErroSemantico(self, message):
        print("\n" + Fore.RED  + "[Erro Semantico] na linha " + str(self.token.getLinha()) + " e coluna " + str(
            self.token.getColuna()) + ": ")
        print(message, "\n", Fore.WHITE)

    def sinalizaErroSintatico(self, message):
        print("\n" + Fore.RED + "[Erro Sintatico] na linha " + str(self.token.getLinha()) + " e coluna " + str(
            self.token.getColuna()) + ": ")
        print(message, "\n", Fore.WHITE)
    ...
    def TipoPrimitivo(self):
        ...
        if self.token.getNome() == Tag.KW_BOOL:
            ...
            self.sinalizaErroSintatico(
                "Esperado \"bool, integer, String, double e void\"; encontrado " + "\"" + self.token.getLexema() + "\"")
    ...
    def Exp4(self):
        if self.eat(Tag.ID):
            ...
            self.Expressao()
            if not self.eat(Tag.OP_FPA): self.sinalizaErroSintatico(')', '')
        elif self.token.getNome() == Tag.OP_NEGACAO or self.token.getNome() == Tag.OP_UNARIO_DIFERENTE:
            self.OpUnario()
            self.Exp4()
        elif (not self.eat(Tag.NUM_INTEIRO) and not self.eat(Tag.NUM_DOUBLE) and not self.eat(
                Tag.LIT) and not self.eat(Tag.KW_TRUE) and not self.eat(Tag.KW_FALSE)):
